type PhotoCardProps = {
  ...
  function PhotoCard({ thumbnailUrl, title, id }: PhotoCardProps) {
    let newImageUrl = thumbnailUrl.replace('150', '256x200').concat(`/ffffff?text=Photo ${id}`)
  
    return (
      <div className='w-full sm:w-80 bg-gray-50 rounded rounded-t-lg shadow-md flex flex-col gap-4 cursor-pointer'>
